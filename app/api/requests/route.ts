...
export async function GET(request: NextRequest) {
  try {
    ...
    // if participantId is provided or status or ride is OPEN which requires validation to view, returns True
    const needsSession = Boolean(participantId) || statusFilters.includes("OPEN");
    ...
    if (needsSession) {
      ...
      if (participantId && user.id !== participantId) {
        return NextResponse.json(
          { error: "Unauthorized ride participant access." },
          { status: 403 }
        );
      }

      // if the user is not a driver, don't show them OPEN ride requests unless scoped to themselves
      if (statusFilters.includes("OPEN") && !participantId && !user.driverInfo) {
        return NextResponse.json(
    ...
    const orderBy =
      statusFilters.includes("OPEN") && !driverId && !participantId
